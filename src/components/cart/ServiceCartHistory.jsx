...
export default function ServiceCartHistory({ setCartView }) {
  ...
  async function loadServiceQuotation(quotationNo, isPrinting = false) {
    try {
      setError("");

      // 1) Load header
      const { data: header, error: headerError } = await supabase
        .from("service_quotation_headers")
        .select("*")
      ...
      if (headerError) {
        throw new Error(
          headerError.message || "Failed to load service quotation header"
        );
      }
      ...
      if (linesError) {
        throw new Error(
          linesError.message || "Failed to load service quotation lines"
        );
      }

      // 3) Restore header fields into quoteDetails
      setQuoteDetails((prev) => ({
        ...
        designationOfUser: isPrinting
          ? header.prepared_by_designation ?? ""
          : prev.designationOfUser,
        frName: isPrinting ? header.from_name ?? "" : prev.frName,
      }));

      // 4) Set quotation number
      // setQuoteNum(header.quotation_no ?? "");

      // 5) Restore service rows
      const mappedRows =
        (lines || []).map((line) => ({
          id: crypto.randomUUID(),
      ...
      setRowsService(mappedRows.length > 0 ? mappedRows : getDefaultServiceRows());
    } catch (err) {
      console.error(err);
      setError(err.message || "Failed to load service quotation");
      alert(err.message || "Failed to load service quotation");
    }
  }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (error) {
          throw new Error(
            error.message || "Error loading service quotation history"
          );
        }
  ...
  return (
    <div>
      <div className="h-24 flex justify-between items-center px-32">
        <h1 className="text-3xl font-bold">Service Quotation History</h1>
                    ...
                    <td>
                      <div className="flex flex-row justify-end gap-2">
                        <SquarePen
                          ...
                          onClick={async () => {
                            await loadServiceQuotation(q.quotation_no, false);
                        ...
                        <Printer
                          ...
                          className="hover:text-[#3cb54c] cursor-pointer hover:scale-125 h-4 w-4"
                          onClick={async () => {
                            await loadServiceQuotation(q.quotation_no, true);
